...
export const AbilitiesPlugin = async (ctx) => {
    const abilities = new Map();
    const pendingCheckpointSummaries = new PendingCheckpointSummaries();
    const pendingFocusTriggers = new Map();
    const lastActiveStepBySession = new Map();
    ...
    const estimateOutputSize = (payload) => {
    ...
    const executeAbilityByName = async (name, inputs = {}, runtime) => {
    ...
    const executeBibliographyCommand = async (command, args, runtime) => {
        ...
        try {
            const stageResult = await bibliographyPipeline.runStageCommand(routed.stage, ability, routed.inputs, createExecutorContext(runtime), { executorOptions: { eventBus } });
            return JSON.stringify({
    ...
    return {
        ...
        async 'chat.message'(input, output) {
            try {
                ...
                const injections = [];
                const pendingSummary = pendingCheckpointSummaries.consume(typeof sessionID === 'string' ? sessionID : undefined);
                if (pendingSummary) {
        ...
        async 'tool.execute.before'(input, _output) {
            try {
                ...
                if (!execution)
                    return; // No ability running, allow all tools
                const currentStep = execution.currentStep;
                if (!currentStep)
                    return;
                // Always allow read-only tools
                if (ALWAYS_ALLOWED_TOOLS.includes(input.tool))
                    return;
                // Script steps block ALL other tools (deterministic execution)
                if (currentStep.type === 'script') {
                    throw new Error(`[abilities] Tool '${input.tool}' blocked during script step '${currentStep.id}'. ` +
        ...
        async 'tool.execute.after'(input, output) {
            try {
                ...
                const outputSize = estimateOutputSize(output?.output ?? output);
        ...
        tool: {
            ...
            'ability_run': tool({
                ...
                async execute({ name, inputs = {} }, toolCtx) {
                    return executeAbilityByName(name, inputs, { sessionID: toolCtx.sessionID, agent: toolCtx.agent });
                },
            }),
            'ability_command': tool({
                ...
                async execute({ command, arguments: commandArguments }, toolCtx) {
                    ...
                    return executeBibliographyCommand(command, parsedArgs, { sessionID: toolCtx.sessionID, agent: toolCtx.agent });
                },
            }),
            ...
            'ability_context_detail': tool({
                ...
                async execute({ use_case }) {
                    ...
                    const selected = selectDetailFields(detail, use_case);
                    return JSON.stringify({
                        status: 'ok',
                        topic,
                        use_case,
            ...
            'ability_coding_artifacts': tool({
                ...
                async execute({ action, type, key }) {
                    const isValidType = (value) => {
                        return CODING_ARTIFACT_TYPES.includes(value);
                    };
                    if (action === 'list') {
                        if (type) {
                            if (!isValidType(type)) {
                        ...
                        const allByType = await Promise.all(CODING_ARTIFACT_TYPES.map(async (artifactType) => ({
                            type: artifactType,
