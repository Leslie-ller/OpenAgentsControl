...
function interpolateVariables(text, inputs, completedSteps) {
    if (!text)
        return text;
...
function truncateForContext(value, maxLength = 50000) {
    if (value.length <= maxLength)
        return value;
    return value.slice(0, maxLength);
}
function buildScriptContextEnv(execution, ctx) {
    const stepOutputs = Object.fromEntries(execution.completedSteps.map((step) => [
        step.stepId,
        {
            status: step.status,
...
async function executeScriptStep(step, execution, ctx) {
...
async function executeAgentStep(step, execution, ctx) {
    const startedAt = Date.now();
        ...
        for (const depId of step.needs) {
            ...
            if (depResult?.output) {
                ...
                if (output) {
                    ...
                    const depStep = execution.ability.steps.find(s => s.id === depId);
    ...
    if (Object.keys(stageOutputs).length > 0) {
        const serializedStageOutputs = truncateForContext(JSON.stringify(stageOutputs, null, 2));
...
async function executeSkillStep(step, execution, ctx) {
    const startedAt = Date.now();
    if (!ctx.skills?.load) {
        return {
            stepId: step.id,
...
async function executeApprovalStep(step, execution, ctx) {
    const startedAt = Date.now();
...
async function executeWorkflowStep(step, execution, ctx) {
    const startedAt = Date.now();
    if (!ctx.abilities) {
        return {
            stepId: step.id,
...
function evaluateCondition(condition, inputs, completedSteps) {
...
function buildExecutionOrder(steps) {
    const result = [];
    ...
    while (remaining.length > 0) {
        ...
        if (!next) {
            ...
            throw new Error(`[abilities] Circular or unresolvable step dependency detected. ` +
 ...
 */
async function runHooks(hooks, ctx, label) {
...
export async function executeAbility(ability, inputs, ctx, options) {
    ...
    for (let i = 0; i < orderedSteps.length; i++) {
        ...
        if ('when' in step && step.when) {
            ...
            if (!conditionMet) {
                ...
                if (eventBus && eventFactory) {
                    eventBus.emit(eventFactory.stepCompleted(ability.name, step.id, step.type, 'skipped', 0, { tags: step.tags }));
                }
                // Fire onStepComplete callback
                if (ctx.onStepComplete)
                    ctx.onStepComplete(step, skippedResult, execution);
                continue;
            }
        }
        // Emit step.started
        ...
        if (ctx.permissionValidator && ctx.agentPermissions) {
            ...
            if (!permResult.allowed) {
                ...
                if (eventBus && eventFactory) {
                    eventBus.emit(eventFactory.stepFailed(ability.name, step.id, step.type, 0, permError, step.tags));
                }
                // Fire onStepFail callback
                if (ctx.onStepFail)
                    ctx.onStepFail(step, permError, execution);
        ...
        if (eventBus && eventFactory) {
            if (result.status === 'failed') {
                eventBus.emit(eventFactory.stepFailed(ability.name, step.id, step.type, result.duration, result.error, result.tags));
            }
            else {
                eventBus.emit(eventFactory.stepCompleted(ability.name, step.id, step.type, result.status, result.duration, { tags: result.tags, output: result.output }));
            }
            ...
            if (step.type === 'agent' && result.modelAudit) {
                ...
                const drift = hasModelDrift({ model: audit.expectedModel, provider: audit.expectedProvider }, { model: audit.actualModel, provider: audit.actualProvider });
                eventBus.emit(eventFactory.modelObserved(ability.name, drift, 'agent_step_dispatch', {
            ...
            if (step.validation?.exit_code !== undefined) {
                const exitCodeValidation = step.validation;
                eventBus.emit(eventFactory.validationResult(ability.name, step.id, 'script.exit_code', exitCodeValidation.exit_code, result.status === 'completed' ? exitCodeValidation.exit_code : 'non-zero', result.status === 'completed'));
            }
            // Emit obligation.signal for steps with tags (evidence that an obligation-related action occurred)
            if (result.tags && result.tags.length > 0) {
                for (const tag of result.tags) {
                    eventBus.emit(eventFactory.obligationSignal(ability.name, tag, // the tag itself serves as evidence key
                    'step_completed', {
                        step_id: step.id,
                        step_type: step.type,
            ...
            if (evidenceStats) {
                eventBus.emit(eventFactory.evidenceStats(ability.name, evidenceStats, { step_id: step.id }));
            }
        }
        if (result.status === 'failed') {
            // Fire onStepFail callback
            if (ctx.onStepFail)
                ctx.onStepFail(step, result.error || 'Unknown error', execution);
    ...
    if (eventBus && eventFactory) {
        ...
        eventBus.emit(eventFactory.runCompleted(ability.name, finalStatus, duration, execution.control?.gate.verdict));
    }
    return execution;
}
function extractEvidenceStats(output) {
